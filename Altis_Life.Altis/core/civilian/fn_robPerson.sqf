...
if(CASH > 0) then {
	...
	if(life_HC_isActive) then {
		[getPlayerUID _robber,_robber GVAR ["realname",name _robber],"42"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
	} else {
		[getPlayerUID _robber,_robber GVAR ["realname",name _robber],"42"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
	};
