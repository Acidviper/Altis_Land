...
if(count _invs > 0) then {
	...
	if(_illegal > 6000) then {
		...
		if(life_HC_isActive) then {
			[getPlayerUID _civ,_civ GVAR ["realname",name _civ],"39"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
		} else {
			[getPlayerUID _civ,_civ GVAR ["realname",name _civ],"39"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
		};
		
	};

	if(life_HC_isActive) then {
		[getPlayerUID _civ,_civ GVAR ["realname",name _civ],"38"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
	} else {
		[getPlayerUID _civ,_civ GVAR ["realname",name _civ],"38"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
	};
