...
if((player distance (getMarkerPos "artefact_1") < 30) || (player distance (getMarkerPos "artefact_2") < 40)) then
{
    _handle = [] spawn life_fnc_gather;
    waitUntil {scriptDone _handle};
    life_action_gathering = false;
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	if(!dialog && playerSide == west && cursorTarget distance player < 4.0) then {
	...
	if((_curTarget GVAR ["civrestrained",false]) && {!(player GVAR ["restrained",false])} && !dialog && playerSide == civilian) exitWith
	{
	    [_curTarget] call life_fnc_civInteractionMenu;
	};
	
	//Set menu Medic
	if(!dialog && playerSide == independent && cursorTarget distance player < 4.0) then {
