...
if(!isNull _killer && {_killer != _unit} && {side _killer != west} && {alive _killer}) then {
	if(vehicle _killer isKindOf "LandVehicle") then {
		...
		if(life_HC_isActive) then {
			[getPlayerUID _killer,_killer GVAR ["realname",name _killer],"66"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
		} else {
			[getPlayerUID _killer,_killer GVAR ["realname",name _killer],"66"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
		};
		...
		if(life_HC_isActive) then {
			[getPlayerUID _killer,_killer GVAR ["realname",name _killer],"65"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
		} else {
			[getPlayerUID _killer,_killer GVAR ["realname",name _killer],"65"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
		};
